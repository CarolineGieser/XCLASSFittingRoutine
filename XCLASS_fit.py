...
for z in range(tag.size):
	...
	for k in range(mol_name_file.size):
		...
		for j in range(cores.size):
			...
			#XCLASS setup files
			MolfitsFileName = str(working_directory) + 'FITS/molecules_' + str(mol_name_file[k]) + '.molfit'
			experimentalData = str(working_directory) + 'FITS/observation_' + str(cores[j]) + '_' + str(number[j]) + '_' + str(mol_name_file[k]) + str(tag[z]) + '.xml'
