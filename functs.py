...
def plot_results_molecule(mol_name_file):
	...
	for k in range(mol_name_file.size):
		...
		plt.hist(T, range=(0.0,300.0), log=False, color='firebrick', label=None)
		plt.xlabel('Temperature [K]')
		ax = plt.subplot(2, 2, 2)
		plt.hist(np.log10(N), range=(12.0,19.0), log=False, color='steelblue', label=None)
		plt.xlabel('LOG(Column Density) [LOG(cm$^{-2}$)]')
		ax = plt.subplot(2, 2, 3)
		plt.hist(delta_v, range=(0,10), log=False, color='seagreen', label=None)
		plt.xlabel('Line Width [km s$^{-1}$]')
		ax = plt.subplot(2, 2, 4)
		plt.hist(v_off, range=(-1,1), log=False, color='darkorange', label=None)
